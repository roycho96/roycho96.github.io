...
> **Version note.** 코드 인용과 line number 는 [NVIDIA/nccl `v2.30.4-1` tag](https://github.com/NVIDIA/nccl/releases/tag/v2.30.4-1) 기준. `master` 는 움직이는 브랜치라 시간이 지나면 line 과 일부 gating 상수가 바뀔 수 있다. 단 §7 의 RTX 5060 Ti 로그는 NCCL 2.28.9 환경에서 얻은 **관찰 방법론 예시**이며 v2.30.4-1 의 cost table 을 재현한 결과는 아니다. 2.28 → 2.30 사이에 실제로 바뀐 부분은 (a) Hopper+ mixed-compcap 에서의 LL128 enable 완화, (b) `perChMaxRingLL128Bws` 의 Blackwell row 정도이고 `NCCL_LL128_C2C` 기본값 (1) · `baseLatencies` · `hwLatencies` · `nvlsEfficiency` 는 동일하다.
...
Cost model 은 통신 한 번이 걸리는 시간을 모델링한 것. 알고리즘 A 와 B 중 어느 게 빠를지 비교하거나 NCCL 같은 라이브러리가 자동 선택을 할 때 쓰는 도구다. NCCL 의 per-call 최종 비교식도 결국 한 줄짜리 `time = lat·latCount + bytes / bw` (§6.4) — 단 그 한 줄의 입력인 `lat` 와 `bw` 는 topology search, protocol efficiency, algorithm 별 보정 계수, channel 수 heuristic 이 이미 누적된 값이다. 이 글 §4 의 algorithm 별 분석도 다 그 cost model 위에서 움직인다.
...
```c
// src/init.cc:51-53 — 세 string array 정의
...
`src/include/device.h` 에는 `extern` 선언만 있고 정의는 `src/init.cc` 에 있다. 그리고 id 상수 자체는 plugin header 에 `#define` 으로 박혀 있다 — tuner plugin 작성자가 알고리즘/프로토콜 번호를 직접 받아야 하기 때문.

```c
// src/include/plugin/nccl_tuner.h:28-40 — algorithm / protocol id
...
PAT 가 잘 안 보이는 이유. `ncclPatEnable` (`src/graph/tuning.cc`) 가 세 조건을 다 요구한다.
...
- 채널 ≥ 2 개.
- Simple protocol 만. `tuning.cc:288` 의 `if ((a == NCCL_ALGO_NVLS || a == NCCL_ALGO_NVLS_TREE) && p != NCCL_PROTO_SIMPLE) continue;` 가 LL / LL128 셀을 bandwidth 테이블 빌드 단계에서 건너뜀.
- **단일 노드: `NVLS` 만 후보** (`NVLS_TREE` 는 `comm->nNodes == 1` 에서 자동 disable, `tuning.cc:474`).
...
- **dtype / reduction op 도 조건**. AllGather 를 제외한 collective 는 `ncclNvlsSupported` (`src/include/device.h:574-589`) 를 한 번 더 통과해야 NVLS 후보가 된다. 매트릭스가 좁다:
...
즉 int32/uint32/int64/uint64/fp16/bf16 는 sum/min/max 까지, fp32/double 은 **sum 만**, 나머지 (fp8 등) 는 NVLS 후보 자체가 안 된다. 일반적인 sum 기반 학습 workload 에서는 NVLS 가 강한 후보가 자주 되는데, prod=곱셈 reduction 이나 fp8 mixed-precision 같은 변종에서는 NVSwitch 가 있어도 NVLS 가 빠진다.
...
Inter-node 비용이 `interLat × 1` 한 번. 노드 사이 reduction 이 switch 안에서 끝나기 때문.

조건은 "SHARP NIC 만 있으면 된다" 보다 훨씬 까다롭다. `tuning.cc` 의 gating 을 코드 순서대로 보면:

```c
// src/graph/tuning.cc:282-284 — CHAIN 은 AG/RS 자체에서 배제
if ((coll == ncclFuncReduceScatter || coll == ncclFuncAllGather)
    && a != NCCL_ALGO_PAT && a != NCCL_ALGO_RING
    && a != NCCL_ALGO_NVLS && a != NCCL_ALGO_COLLNET_DIRECT) continue;

// src/graph/tuning.cc:334-350 — multi-node AG/RS 의 plugin support
if ((a == NCCL_ALGO_NVLS || a == NCCL_ALGO_COLLNET_DIRECT) && p == NCCL_PROTO_SIMPLE
  ...
  // 각 노드의 localRanks 가 nHeads 보다 크면 busBw = 0
...
// src/graph/tuning.cc:476-479 — collnetEnable 마스터 스위치
...
// src/graph/tuning.cc:481 — NVSwitch 가 없으면 COLLNET_DIRECT 만 disable (CHAIN 은 통과)
if (nvsCount == 0 && a == NCCL_ALGO_COLLNET_DIRECT) disable = 1;
```

읽으면:

- **CollNetChain 은 AllReduce 전용**. AG/RS 분기에서 `continue` 로 일찍 빠진다 (`:282-284`).
- **`collnetEnable=0` 이면 둘 다 disable** + multi-node NVLS 도 같이 disable.
- **CollNetDirect 은 NVSwitch 가 있어야 한다.** `nvsCount == 0` 이면 disable.
- **multi-node AG/RS 의 CollNetDirect 은 plugin `iallgather` / `ireducescatter` 함수가 등록돼야 함** + 각 노드의 `localRanks ≤ collNetHeadsNum` 까지 만족해야 살아남는다.

즉 "SHARP 가 있으면 자동" 이 아니라 NCCL 이 CollNet transport, plugin 함수, NVSwitch, head 수 까지 다 만족한다고 판단할 때만 후보가 된다. HPC InfiniBand SHARP 환경에서 의미 있고, RoCE 나 NVLink 직결 환경에서는 NVLS 가 더 자주 골라진다.

> Direct 와 Chain 의 차이: Direct 는 노드 안의 모든 GPU 가 head NIC 으로 직접 묶이는 "fan-in" 레이아웃이라 leaf reduce 가 한 hop 으로 끝난다 (식의 `min(1, p_node-1)·intraLat`). Chain 은 노드 안에서 GPU 가 일렬로 reduce-up 한 뒤 head 가 inter 로 나가는 "체인" 형태라 intra latency 가 `(p_node-1)·intraLat` 로 linear. NVSwitch 노드에선 Direct 가 거의 항상 우세하지만 PCIe-only 노드라 NVSwitch 가 없으면 Chain 만 남는다.
...
LL128 은 atomicity 가 보장되는 transport 에서만 enable 된다. 조건이 꽤 까다롭다 (`src/graph/tuning.cc:488-502`).
...
if (pEnable == 2 && p == NCCL_PROTO_LL128) {
  ...
  if (ncclParamLl128C2c() && minCompCap >= 90) {
    // Hopper+ 에서는 기본으로 PXN 까지 (C2C/PXN 포함)
    pEnable &= (graphs[a]->typeInter <= PATH_PXN);
  } else {
    pEnable &= (graphs[a]->typeInter <= PATH_PXB);
  }
  pEnable &= (graphs[a]->typeIntra <= PATH_NVB);    // intra 는 NVLink 계열만
  pEnable &= (minCompCap == maxCompCap || minCompCap >= 90);  // compcap uniform 또는 모두 ≥ Hopper
}
```

`NCCL_LL128_C2C` 의 기본값은 **1** (`tuning.cc:229`). 즉 사용자가 의식적으로 켜야 하는 옵션이 아니라 **기본으로 켜져 있고**, `NCCL_LL128_C2C=0` 으로 끌 때 inter path 가 `PATH_PXB` 까지로 좁아진다. 정리하면:

- Intra-node 는 `PATH_NVB` 이하 (NVLink 직결만). PCIe 거치면 LL128 안 씀.
- Inter-node 는 (Hopper/Blackwell + LL128_C2C=기본 1) → `PATH_PXN` 까지. `LL128_C2C=0` 이거나 Hopper 미만 → `PATH_PXB` 까지.
- Compute capability 가 uniform 이거나 모두 ≥ Hopper.

> v2.28 → v2.30 사이에 마지막 조건이 한 글자 늘었다. v2.28 에서는 `(minCompCap == maxCompCap)` 만 요구했는데 v2.30 부터는 `|| minCompCap >= 90` 이 붙어서 **Hopper/Blackwell 끼리는 compcap 이 섞여도 LL128 이 enable** 된다. 그 외 (`NCCL_LL128_C2C` 기본값, 위 분기 구조) 는 두 tag 가 동일.
...
> **Scope.** 이하 설명은 NCCL **built-in tuner** 기준이다. `NCCL_TUNER_PLUGIN` 으로 외부 tuner 를 붙이면 동작이 달라진다: `enqueue.cc:2051-2066` 에서 NCCL 이 먼저 built-in cost table 을 채워 plugin 의 `getCollInfo(..., float** collCostTable, ...)` 에 **InOut** 으로 전달하고, plugin 이 그 표를 in-place 로 수정한 뒤 NCCL 이 동일한 `topoGetAlgoInfo` argmin 을 그 위에서 돈다. 즉 외부 tuner 는 **completely replace** 가 아니라 **re-rank/override** 로 들어온다. 그리고 직접 C plugin 을 작성하지 않더라도 NCCL repo 의 [`plugins/tuner/example/`](https://github.com/NVIDIA/nccl/tree/v2.30.4-1/plugins/tuner/example) 가 `NCCL_TUNER_CONFIG_FILE` 로 CSV 기반 size/topology 별 override 를 지원한다 (env var 자체는 NCCL core 가 아니라 이 example plugin 안에서 parse 됨).

### 6.1 Init: α/β 테이블 빌드

`ncclTopoTuneModel` (`src/graph/tuning.cc`) 이 communicator init 시 모든 (collective, algorithm, protocol) 셀에 대해 두 표를 채운다.
...
사용자가 `ncclAllReduce(...)` 를 부르면 collective task 가 만들어지고, `topoGetAlgoInfo` (`src/enqueue.cc`) 가 message size + group size 를 보고 §1 의 7 × 3 표에서 argmin 을 찾는다.
...
`ncclTopoGetAlgoTime` 의 **최종 line** 은 한 줄이다.
...
$T = (\text{lat} \times \text{latCount}) + \frac{\text{nBytes}}{1000 \times \text{bw}}$ — 단순한 $\alpha + n\beta$ 의 NCCL 구현. 그러나 한 줄로 보이는 이 식의 입력 `lat`, `bw` 는 위로 거슬러 가면 heuristic 누적이다 (`tuning.cc:312-329`, `:601-605`):

- **bw 보정** (`ncclTopoTuneModel` 안): Ring LL `× 0.5` capped at `llMaxBw`, Ring LL128 `× 0.92` capped, Tree+AllReduce `× 0.92`, Tree LL `× 1/3.8`, Tree LL128 `× 7/9` (단일 노드) 또는 `× 120/128` (multi-node), Tree `NCCL_TOPO_PATTERN_TREE` 추가 `× 0.85`, PAT `× 0.75`, CollnetDirect Simple+Hopper `× 0.85`.
- **bw 추가 보정** (`ncclTopoGetAlgoTime` 안): Tree AllReduce 와 NVLS_TREE (Blackwell+x86) 는 size-별 `treeCorrectionFactor[protocol][logSize]` table 한 번 더.
- **lat 보정**: Ring + Simple + multi-node + AllReduce + chunk ≥ 64 B 면 `lat *= (minCompCap < 80 ? 1.9 : 1.4)` 의 "Ring plateau" 보정 (`tuning.cc:603-605`).
- **latCount**: Ring 은 `numPipeOps`, 나머지는 `DIVUP(numPipeOps, NCCL_MAX_DEV_WORK_BATCH_COLLS)`.

이 모든 보정을 다 먹은 뒤의 최종 비교식이 그 한 줄. "한 줄" 은 마지막 step 의 형식이고, table 자체는 algorithm × protocol 별 다른 식으로 채워졌다고 보는 게 맞다.
...
§6 의 selection 머신이 실제로 어떤 표를 만들고 어떤 셀을 고르는지는 모델 계산보다 NCCL 자신의 로그를 보는 게 빠르다.

> **이 절의 위치.** 이 절의 로그는 v2.30.4-1 의 cost table 을 재현하는 검증이 **아니다**. 환경은 RTX 5060 Ti × 2 (consumer GPU, P2P 비활성, SHM transport, **NCCL 2.28.9**) 이고, 글 전체의 코드 인용 (v2.30.4-1) 과 minor version 이 다르다. 같은 hardware 라도 NCCL 버전 / CUDA 버전 / P2P 설정 / driver / rank mapping 에 따라 표와 선택이 달라질 수 있다. 이 절의 목적은 NCCL cost model 의 절대값 정확도 검증이 아니라, `NCCL_DEBUG_SUBSYS=TUNING` 으로 **어떤 정보를 어떻게 관찰** 하는지의 methodology 예시.

다음 4 줄이면 §6.1 의 init 테이블 + §6.4 의 dispatch 가 그대로 찍힌다.
...
NCCL 의 SYS-path 기본 bw (0.1 GB/s) 가 매우 보수적인 lower bound 라 절대값은 실측과 큰 차이. 이 예제에서는 살아남은 후보가 Ring LL / Ring Simple 정도라 SYS-path bandwidth 추정의 절대 오차가 최종 선택을 바꾸지 않았을 뿐, 일반적으로 cost model 의 절대/상대 오차가 후보가 많은 NVLink/NVSwitch/IB 환경의 **경계 메시지 크기** 에서는 선택을 바꿀 수 있다. NVSwitch + NVLink 환경 (H100 / H200) 에서 같은 명령을 돌리면 NVLS / NVLSTree 가 살아나고 bw 가 100 ~ 900 GB/s 대로 들어가 Ring 을 밀어낸다 — methodology 는 동일.
...
| 주제 | 파일 | 함수 / 영역 |
|---|---|---|
| Algorithm / Func / Proto 문자열 | `src/init.cc:51-53` | `ncclFuncStr`, `ncclAlgoStr`, `ncclProtoStr` |
| Algorithm / Protocol id | `src/include/plugin/nccl_tuner.h:28-40` | `NCCL_ALGO_*`, `NCCL_PROTO_*` (#define) |
| Device-side extern 선언 | `src/include/device.h` | `extern const char* ncclAlgoStr[]` 등 |
| Topology pattern | `src/include/graph.h` | `NCCL_TOPO_PATTERN_*` |
| PATH 타입 | `src/include/graph.h:110-146` | `PATH_*` (#define, LOC..DIS) |
...
| LL128 enable | `src/graph/tuning.cc:488-502` | `pEnable` 분기 |
| `NCCL_LL128_C2C` 기본값 | `src/graph/tuning.cc:229` | `NCCL_PARAM(Ll128C2c, "LL128_C2C", 1)` |
...
| Cost model 최종식 | `src/graph/tuning.cc:601-609` | `ncclTopoGetAlgoTime` |
| bw / lat 보정 | `src/graph/tuning.cc:312-329, 601-605` | Ring/Tree/PAT/CollnetDirect 보정 + Ring plateau |
...
| CSV-based tuner 예제 | `plugins/tuner/example/` | `NCCL_TUNER_CONFIG_FILE` (plugin 안에서 parse) |
...
- Hu, Z. et al. "Demystifying NCCL: An In-depth Analysis of GPU Communication Protocols and Algorithms." arXiv:2507.04786, 2025 (NCCL 2.19.1 기준; 본 글의 코드 인용은 NCCL v2.30.4-1 이라 일부 수치 / 분기가 다를 수 있음).
