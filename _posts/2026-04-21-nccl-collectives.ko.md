...
- 양쪽이 buffer readiness 를 합의하는 **rendezvous** 가 데이터 전송 앞에 깔린다.
 ...
 DMA 장치 (GPU, NIC) 는 OS 를 우회해 직접 물리 주소에 access 하니까 page 가 swap 되면 엉뚱한 메모리에 쓰는 사고가 남. 그래서 staging buffer 는 반드시 pinned 여야 한다. 익숙한 비유로, PyTorch DataLoader 의 `pin_memory=True` 가 만드는 게 정확히 같은 메모리 type. 거기서는 dataset → GPU H2D copy 를 빠르게 (그리고 `non_blocking=True` 가 진짜 async 로 동작하게) 하는 용도로 쓰고, NCCL 도 같은 이유로 GPUDirect RDMA 가 안 되는 환경에서 host pinned memory 를 staging buffer 로 둔다. CUDA API 로는 둘 다 `cudaHostAlloc` / `cudaHostRegister`.

#### Topology graph

*Topology graph* 는 한 노드의 GPU, NIC, PCIe bridge, NVSwitch, NUMA 와 그들 사이 link (NVLink, PCIe, C2C 등) 를 노드 / 엣지로 표현한 자료구조다. NCCL 은 init 시 이걸 빌드한다. §4.5 의 intra-node 우선순위와 GPUDirect RDMA 게이팅, §5 의 알고리즘 선택까지 다 이 graph 위에서 결정된다. `ncclTopoGetSystem` (`src/graph/topo.cc`) 이 sysfs 의 `/sys/class/pci_bus`, `/sys/devices/system/node` 와 NVML (NVLink, C2C 정보), 거기에 네트워크 플러그인의 NIC 속성을 종합해 GPU / NIC / NVSwitch / PCIe bridge / CPU-NUMA 노드를 detect 한다. intra-node rank 들은 자기가 본 view 를 bootstrap-allgather 로 합쳐 system 단위 graph 로 통합하고, `ncclTopoComputePaths` (`src/graph/paths.cc`) 가 BFS 로 모든 (source, target) 쌍에 *path type* (`PATH_LOC` < `NVL` < `NVB` < `C2C` < `PIX` < `PXB` < `P2C` < `PXN` < `PHB` < `SYS` < `NET`) 과 aggregate bandwidth 를 라벨링한다.

Transport 선택은 단순하다. `selectTransport` 가 `{p2p, shm, net, collNet}` 순서로 돌면서, `canConnect` 가 처음으로 통과하는 걸 픽한다. `NCCL_P2P_LEVEL` (default `PATH_PXB`) 이 P2P 의 reach 를 cap 하고, `NCCL_NET_GDR_LEVEL` (default `PATH_P2C` if C2C else `PATH_PXB`) 이 GDR 을 cap 한다. 알고리즘 search (`src/graph/search.cc`) 도 같은 graph 위에서 Ring / Tree / NVLS / CollNet 을 score 한다. 본인 시스템에서 NCCL 이 뭘 봤는지 직접 보고 싶으면 `NCCL_TOPO_DUMP_FILE=topo.xml` 을 켜면 rank 0 가 system XML 을 dump 한다. 비표준 chassis 처럼 detection 을 override 해야 할 때는 `NCCL_TOPO_FILE` 에 직접 만든 XML 을 가리키면 된다.
...
§5.1 의 bandwidth-optimal $2(p-1)K/(pB)$ 가 깔끔하게 떨어지는 건 NCCL 이 한 가지 구현 선택을 강제했기 때문이다. send / recv / reduce / forward 를 다 하나의 CUDA kernel 안에 넣는 것. 만약 reduce 를 별개 kernel 로 launch 하는 식으로 짜졌다면, CUDA launch latency 는 매 step 마다 누적되고, launch 사이에 SM 이 비고, reduce kernel 이 방금 도착한 byte 를 HBM 에서 다시 읽어야 한다. NCCL 은 ring 한 바퀴 + reduction 까지 single kernel 안에 다 묶었다. launch 는 1 회로 끝난다. `recv → reduce → send` 가 같은 thread 의 register set 안에서 끝나서 byte 가 stage 사이 HBM 을 안 거친다 (§5.3, §7 Layer 2). §5.0 의 channel 모델도, 아래 코드 dive 도 다 여기서 나온다.
